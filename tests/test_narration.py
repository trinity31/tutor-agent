...
def test_조사_붙은_한자_복합어는_보존():
    # 순수 한자 복합어 + 한글 조사는 낭독에서 독음으로 읽어야 하므로 삭제하지 않는다.
    # (과거: 앞 글자 유실로 丙火가→화가, 己土가→토가가 되던 버그)
    assert clean_text("10일간은 丙火가 사령한다") == "10일간은 丙火가 사령한다"
    assert clean_text("강한 午火가 子水를 본다") == "강한 午火가 子水를 본다"


def test_벌려_추출된_한자_복합어_복원():
    # 슬라이드형 PDF가 "丙火"를 "丙 火"로 벌려 추출해도, 붙여서 복합어로 보존한다
    assert clean_text("10일간은 丙 火가 사령한다") == "10일간은 丙火가 사령한다"
    assert clean_text("강한 午 火가 子 水를 본다") == "강한 午火가 子水를 본다"
