...
요약본/정리본이 아니다. 각 섹션이 무엇을 다루는지만 짧게 적는다.

**최우선 원칙**: 자료의 **원본 번호/제목 체계를 그대로 보존**하고, **들여쓰기로 계층**을 표현한다.

**분량 상한**: PDF 페이지 수의 2~3배 줄 이내 (예: 10페이지 → 25줄 내외)

**형식 규칙**:

1. 자료의 **최상위 장(章·대제목)만** `##` 마크다운 헤딩으로 표시한다. 그 이하는 절대 ###/#### 같은 헤딩을 추가하지 말고, **자료 원본의 번호 체계(1., 1), (1), ① 등)를 그대로** 사용한다.
2. 항목이 단일 포인트면 → `번호) 항목명: 한 줄 요약 또는 키워드 3~7개 콤마 나열`
3. 항목이 여러 핵심 포인트(2~5개)를 가지면 → 다음 줄부터 4-space 들여쓰기 후 `- 이름: 키워드 2~5개` 한 줄씩.
4. 자료의 계층 깊이는 들여쓰기(4 space)로 표현. 같은 깊이는 같은 들여쓰기.
5. 핵심 용어, 인명, 연도, 한자 원어, 외래어는 보존한다 (예: 부르디외(Bourdieu, 1930-2002)).
6. 도식·그림·각주는 거의 다 생략.
7. JSON·코드블록·인사말 없이 마크다운 본문만 출력. 첫 줄은 `# {자료 제목}`.
...
1) 형국론의 부동산 풍수적 의미
    ...
    - 도시·부동산 풍수에서의 형국론 적용
2) 대산용도와 의지용도
    - 대산용도(론스타): 태조산, 큰 기운, 주산
    - 의지용도: 큰산용도에 기대어 생존
3) 공생용도: 궁합 맞는 것끼리 공생
4) 은하수용도: 1종 근린생활시설
5) 용도풍수의 새로운 명칭과 성격 요약
6) 발산용도와 흡인용도: 배달형, 테이블형, 이벤트형, 유인형
7) 토지의 용도 - 지목
    - 지목의 기운

**금지 패턴** (절대 이렇게 쓰지 말 것):

### 1. 용도의 형국론적 이해: 형국론, 대산용도, 공생용도, 발산용도
(원본 번호 체계 무시하고 ###로 모두 같은 헤딩 레벨에 욱여넣기 — 금지)
