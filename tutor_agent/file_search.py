...
_INDEX_PROMPT = """첨부된 PDF 강의자료를 한눈에 훑을 수 있는 **목차형 인덱스**로 정리해줘.
이건 요약본이 아니라 인덱스다. 본문을 풀어쓰지 말고, 무엇을 다루는지만 짧게 적는다.

분량 목표: PDF 본문의 10% 이내 (페이지당 2~5줄)

형식 규칙:
1. 자료의 헤딩 구조(장·절·항)는 ##, ### 으로 보존한다.
2. 각 항목 본문은 **한 줄 요약** 또는 **핵심 키워드 콤마 나열** 둘 중 하나로만 적는다.
3. 한 불릿 = 한 줄. 본문 문장을 그대로 옮기거나 풀어쓰지 않는다.
4. 핵심 용어, 인명, 연도, 한자 원어는 보존한다 (예: 주역(周易), 왕필(王弼, 226-249)).
5. 도식·그림은 생략하거나 "[그림: 한 줄 캡션]"으로만 언급한다.
...
## 7. 공장풍수
### 1) 입지 조건
풍살(風殺)·수살(水殺) 회피, 투자 이익, 기반시설 용이, 민원 우려 없음
### 2) 구축 유형
- 파빌리온 타입: 독립형 동, 증축 용이, 마스터플랜 없으면 기운 혼탁
- 블록 클러스터 타입: 블록 단위 연결

나쁜 예 (절대 이렇게 하지 말 것):
### 1) 입지 조건
- 풍살(風殺)과 수살(水殺)을 피할 수 있는 곳.
- 향후 지가 상승으로 인한 부동산 투자의 이익 도모 가능.
- 기반 시설 이용이 손쉬운 곳.
"""
